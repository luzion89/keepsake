...
- 系统 PATH 中的 `cloudflared` 二进制（需用户预先安装，见先决条件）
...
│   └── auth.ts         # 所有 /auth/* 路由
└── tunnel/
    └── cloudflared.ts  # 系统 PATH spawn，URL 捕获，进程清理，ENOENT fallback
```
...
### 先决条件：装 cloudflared 到系统 PATH

Keepsake 通过调用系统 PATH 中的 `cloudflared` 二进制来建立隧道，**不再依赖 npm 包**。请先安装：

- macOS:   `brew install cloudflared`
- Linux:   见 https://pkg.cloudflare.com/index.html
...
若未安装，启动时会打印安装指引并自动切换到 LAN-only 模式（不报错退出）。
...
经过 Spike 一/二/三期实测，**决定不合并到 main，spike 分支保留作为历史**。理由如下：

### 7.1 实测发现

| 验证项 | 结果 |
|--------|------|
| Android Chrome：通过 trycloudflare URL 装 PWA | ✅ 成功 |
| trycloudflare 服务稳定性 | ⚠️ 间歇性 `error code: 1101`（CF 一侧 Worker 异常） |
| iOS Edge / Chrome：装 PWA | ❌ iOS 上所有非 Safari 浏览器都不支持真正的 A2HS（苹果 WebKit 政策） |
| iOS Safari：装 PWA | ⚠️ 可装，但 PWA 模式下样式与浏览器模式有差异，对灵动岛支持不佳 |
| 应用内切换 server | ✅ Settings 增加"设备配对"节，能清 token + 重 pair |
| 邀请二维码扫码 join | ✅ 二维码 + 倒计时 + 复制 fallback |
| family_id 数据隔离 + JWT claim | ✅ |
| 离线 local-first 读写 | ✅ server 离线时 PWA 客户端读写完全正常 |

### 7.2 复杂度成本

为了让 PWA 能装到主屏，spike 引入：
- `cloudflared` 系统二进制依赖（用户需 brew / apt / winget 安装）
- 双端二维码认证（root_secret / device_token / invite_token）
- 多设备 family_id 模型 + JWT 鉴权中间件
- 应用层 E2E 加密（HKDF + AES-256-GCM 字段级密文 + 字段级 LWW）
- PWA Settings 切换 server / 邀请二维码 UI
- server 启动顺序、隧道日志透传等工程处理

这些机制对"个人 + 家庭浏览器访问"场景**收益不足以抵消复杂度**，特别是 iOS PWA 体验本身就不理想。

### 7.3 回退方案（main 实际采用）

回到 main 分支最初的方案：
- **不可信自签名 HTTPS + 浏览器访问**（用户在浏览器红屏点"高级 → 继续"接受）
- **不强求 PWA 安装**（已有的 PWA manifest / Service Worker 代码保留，不删）
- **不引入认证、不引入 E2E、不引入 family_id**
- 单用户 / 同家庭信任 LAN 环境

### 7.4 GitHub Pages 纯前端路径（已评估并放弃）

讨论过"GitHub Pages 托管前端 + 用户运行时配 server URL"方案，被以下硬限制挡住：
- 浏览器 **Mixed Content** 规则：HTTPS 页面禁止 fetch HTTP 接口（仅 `localhost`/`127.0.0.1` 例外）
- LAN IP（192.168.x.x）属于"不可信源"，github.io 上的前端无法 fetch 到本地 HTTP server
- 即便接受"server 必须 HTTPS"，仍要解决证书问题，复杂度并未减少
- 安装到本地的 PWA 受同一规则约束（"装了"不放宽 Mixed Content）
- 自签名 HTTPS：跨 origin fetch 会被静默 reject，没有用户接受 prompt

**结论**：GitHub Pages 路径对"家里有 server，多设备访问"主流场景不成立，正式放弃。

### 7.5 spike 中可单独移植到 main 的小改进

经 CTO 评估，下列条目与 PWA 安装 / 认证 / 隧道无关，独立有价值，可作为独立 issue 合并到 main：

- **服务端定期备份机制**（每 7 天 SQLite 文件 snapshot，本次实操中数据丢失的直接教训）
- **启动时打印 LAN URL**（含本机 IP），方便手机直连，无需 ASCII QR
- 启动顺序 + cloudflared stderr 透传的工程经验（main 不再用 cloudflared，但日志透传思路保留）

### 7.6 spike 资产去向

- **代码**：保留在 `spike/auth-cf-tunnel` 分支，不删除，不合并；
