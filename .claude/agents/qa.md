...
2. **环境问题必须如实报**。cloudflared 装不上、端口被占、build fail、依赖缺失 —— 别说"已 fallback 验证"糊弄过去，如实说"环境 X 缺，未跑通 Y"，escalate 给 PM。
3. **不放过疑点**。复测点不止打勾，要写出具体观察值（"创建房间后 reload，列表仍可见 1 条"），让 PM 能复核。
4. **报告里区分**：
   - "真实跑通且符合预期"
   - "跑了但行为不符（贴日志）"
   - "未跑（说明原因）"
